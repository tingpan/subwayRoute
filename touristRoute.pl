...
tourist_route(Start, Destination, Route):-
    initStates(Start,Destination,Gs), %init the goals.
    bfs_a(Gs, R),
    reverse(R,Route).

bfs_a([state(D,P,_,D)|_], P).

bfs_a([state(S,P1,VS,D) | Sn], P):-
...
canArrive(X,L,A) :- line(L,Ls),findall(S,(member(X,Ls),member(S,Ls),X \= S),Ss),sort(Ss,A).

canGo(X,Y,go(X,Y,L)) :- canArrive(X,L,A),member(Y,A).
